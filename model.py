...
@jit(nopython=True)
def model(delta_t, vars, plas_terms, t, weights, back_inputs,
    ...
    for i in t:
        ...
        if step == int(stim_start * (1 / delta_t)):
            ...
            if (adaptive_LR_mask) and (adaptive_LR_method == "3-factor"):
...
#@jit((float16[:], float16[:], float16[:], float16[:], int16, float[:]),nopython=True)
@jit(nopython=True)
def model_for_eigenvalues(delta_t, vars, t, weights,
              back_inputs, stim_strengths, stim_start, stim_stop, taus, lambdas,
              rheobases, upper_bounds, learning_rates, adaptive_LR_method = "sum", synaptic_scaling_method = "subtractive",
              synaptic_scaling_update_method = "every_timestep",
              synaptic_scaling_compare_method = "individual",
              BCM_p=1, beta_ss=1, ss_exponential=1,flags=(0,0,0,0,0)):
    ...
    theta_I1,theta_I2 = 1,1
    (LR_E01,LR_E02) = learning_rates
    learning_rate_EE11,learning_rate_EE12,learning_rate_EE21,learning_rate_EE22=1,1,1,1
