...
@jit(nopython=True)
def model_3_compartmental(delta_t, hold_every, res_rates, res_weights, sim_duration, weights, back_inputs,
          stim_trains, stim_times, taus, beta_K, rheobases, lambdas,
          flags=(0, 0, 0, 0, 0, 0), flags_theta = (1,1)):
    ...
    (w_DEii, w_EEii, w_EPii, w_DSii, w_PEii, w_PPii, w_PSii, w_SEii,
     w_DEij, w_EEij, w_EPij, w_DSij, w_PEij, w_PPij, w_PSij, w_SEij) = weights
    (g_AD, g_BD, g_E, g_P, g_S, g_top_down_to_S) = back_inputs
    ...
    for step in range(sim_duration):
        if step == int((stim_start + 2) * (1 / delta_t)): # if stim on
            if stim_applied == 0: # if first stim
                ...
                betaAD1, betaAD2, betaBD1, betaBD2, betaE1, betaE2 = I_AD1 - beta_K, I_AD2 - beta_K, I_BD1 - beta_K, I_BD2 - beta_K, I_E1 - beta_K, I_E2 - beta_K
        ...
        if stim_applied == 1 and counter2 == hold_every_simulation:
            ...
            set_phase2[:,i_2] = [thetaAD1, thetaAD2, thetaBD1, thetaBD2, thetaE1, thetaE2, betaAD1, betaAD2, betaBD1, betaBD2, betaE1, betaE2]
            J_phase2[:,i_2] = [DE110, DE120, DE210, DE220, EE110, EE120, EE210, EE220,
                               EP110, EP120, EP210, EP220, DS110, DS120, DS210, DS220]
        ...
        betaAD1 = betaAD1 + adaptive_threshold_mask * delta_t * (1 / tau_beta) * (I_AD1 - betaAD1)
        betaAD2 = betaAD2 + adaptive_threshold_mask * delta_t * (1 / tau_beta) * (I_AD2 - betaAD2)
        betaBD1 = betaBD1 + adaptive_threshold_mask * delta_t * (1 / tau_beta) * (I_BD1 - betaBD1)
        betaBD2 = betaBD2 + adaptive_threshold_mask * delta_t * (1 / tau_beta) * (I_BD2 - betaBD2)
        betaE1 = betaE1 + adaptive_threshold_mask*delta_t * (1 / tau_beta) * (I_E1 - betaE1)
        betaE2 = betaE2 + adaptive_threshold_mask*delta_t * (1 / tau_beta) * (I_E2 - betaE2)

        thetaAD1 = thetaAD1 + adaptive_threshold_mask * delta_t * (1 / tau_theta) * \
                  (-flag_theta_shift * (thetaAD1 - betaAD1) + flag_theta_local * (I_AD1 - thetaAD1))
        thetaAD2 = thetaAD2 + adaptive_threshold_mask * delta_t * (1 / tau_theta) * \
                  (-flag_theta_shift * (thetaAD2 - betaAD2) + flag_theta_local * (I_AD2 - thetaAD2))
        thetaBD1 = thetaBD1 + adaptive_threshold_mask * delta_t * (1 / tau_theta) * \
                  (-flag_theta_shift * (thetaBD1 - betaBD1) + flag_theta_local * (I_BD1 - thetaBD1))
        thetaBD2 = thetaBD2 + adaptive_threshold_mask * delta_t * (1 / tau_theta) * \
        ...
        ss1_W_DE = exc_scal_mask * delta_t * (1 / tau_scaling_E) * ((1 - ratio_AD1)**p_e)
        ss2_W_DE = exc_scal_mask * delta_t * (1 / tau_scaling_E) * ((1 - ratio_AD2)**p_e)

        ss1_W_EE = exc_scal_mask * delta_t * (1 / tau_scaling_E) * ((1 - ratio_BD1)**p_e)
        ss2_W_EE = exc_scal_mask * delta_t * (1 / tau_scaling_E) * ((1 - ratio_BD2)**p_e)

        ss1_W_EP = inh_scal_mask_p*delta_t * (1 / tau_scaling_P) * ((1 - ratio_E1)**p_p)
        ss2_W_EP = inh_scal_mask_p*delta_t * (1 / tau_scaling_P) * ((1 - ratio_E2)**p_p)

        ss1_W_DS = inh_scal_mask_s*delta_t * (1 / tau_scaling_S) * ((1 - ratio_AD1)**p_s)
        ss2_W_DS = inh_scal_mask_s*delta_t * (1 / tau_scaling_S) * ((1 - ratio_AD2)**p_s)
