...
def model(delta_t, hold_every, res_rates, res_weights, sim_duration, weights,back_inputs,
    ...
    for step in range(sim_duration):
        ...
        I1 = x_E - EP110 * P01 - EP120 * P02 - ES110 * S01 - ES120 * S02 + EE110 * E01 + EE120 * E02 + stimulus_E1
        I2 = x_E - EP210 * P01 - EP220 * P02 - ES210 * S01 - ES220 * S02 + EE210 * E01 + EE220 * E02 + stimulus_E2
        ...
        P1 = P01 + delta_t*(1/tau_P)*( -P01 + np.maximum(0, w_PEii * E01 + w_PEij * E02 - w_PSii * S01 - w_PSij * S02
                                                         - w_PPii * P01 - w_PPij * P02 + x_P - rheobase_P + stimulus_P1))
        P2 = P02 + delta_t*(1/tau_P)*( -P02 + np.maximum(0, w_PEij * E01 + w_PEii * E02 - w_PSij * S01 - w_PSii * S02
                                                         - w_PPij * P01 - w_PPii * P02 + x_P - rheobase_P + stimulus_P2))

        S1 = S01 + delta_t*(1/tau_S)*( -S01 + np.maximum(0, w_SEii * E01 + w_SEij * E02 + x_S - rheobase_S + stimulus_S1))
        S2 = S02 + delta_t*(1/tau_S)*( -S02 + np.maximum(0, w_SEij * E01 + w_SEii * E02 + x_S - rheobase_S + stimulus_S2))
        ...
        theta_I1 = theta_I1 + adaptive_threshold_mask*delta_t * (1 / tau_theta) * \
                   (-flag_theta_shift*(1-alpha)*(theta_I1 - beta1) + flag_theta_local*alpha*(E1 - theta_I1))
        theta_I2 = theta_I2 + adaptive_threshold_mask*delta_t * (1 / tau_theta) * \
                   (-flag_theta_shift*(1-alpha)*(theta_I2 - beta2) + flag_theta_local*alpha*(E2 - theta_I2))

        # rates and plasticity thresholds cannot go below 0 (exc ones cannot go below 1e-323, in order to avoid zero division)
        theta_I1=max(theta_I1,1e-10); theta_I2=max(theta_I2, 1e-10)
        ...
        ratio_E1 = max(E1, 1e-2) / max(theta_I1,1e-2); ratio_E2 = max(E2, 1e-2) / max(theta_I2,1e-2)
...
def model_VIP(delta_t, hold_every, res_rates, res_weights, sim_duration, weights,back_inputs,
    ...
    for step in range(sim_duration):
        ...
        S1 = S01 + delta_t*(1/tau_S)*( -S01 + np.maximum(0, w_SEii * E01 + w_SEij * E02 + w_SVii * V01 + w_SVij * V02 + x_S - rheobase_S + stimulus_S1))
        S2 = S02 + delta_t*(1/tau_S)*( -S02 + np.maximum(0, w_SEij * E01 + w_SEii * E02 + w_SVij * V01 + w_SVii * V02 + x_S - rheobase_S + stimulus_S2))
...
def old_model(delta_t, hold_every, vars, plas_terms, sim_duration, weights, back_inputs,
    ...
    (hebEE11, hebEE12, hebEE21, hebEE22, ss1_list, ss2_list,
     av_theta_I1, av_theta_I2, beta1_list, beta2_list) = plas_terms
    theta_I1,theta_I2 = 1,1
    learning_rate = 1
    ...
    for step in range(sim_duration):
        ...
        P1 = P01 + delta_t*(1/tau_P)*( -P01 + np.maximum(0, w_PE11 * E01 + w_PE12 * E02 - w_PS11 * S01 - w_PS12 * S02
                                                         - w_PP11 * P01 - w_PP12 * P02 + x_P - rheobase_P + stimulus_P1))
        P2 = P02 + delta_t*(1/tau_P)*( -P02 + np.maximum(0, w_PE21 * E01 + w_PE22 * E02 - w_PS21 * S01 - w_PS22 * S02
                                                         - w_PP21 * P01 - w_PP22 * P02 + x_P - rheobase_P + stimulus_P2))

        S1 = S01 + delta_t*(1/tau_S)*( -S01 + np.maximum(0, w_SE11 * E01 + w_SE12 * E02 + x_S - rheobase_S + stimulus_S1))
        S2 = S02 + delta_t*(1/tau_S)*( -S02 + np.maximum(0, w_SE21 * E01 + w_SE22 * E02 + x_S - rheobase_S + stimulus_S2))
        ...
        if counter == hold_every:
            ...
            av_theta_I1[i] = theta_I1; av_theta_I2[i] = theta_I2
